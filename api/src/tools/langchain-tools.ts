...
export class LeasingTools {
  ...
  /**
   * Tool for checking unit availability in a community
   */
  checkAvailabilityTool = new DynamicStructuredTool({
    name: "check_availability",
    description: "Check available units in a community, optionally filtered by bedroom count",
    schema: z.object({
      community_id: z.string().describe("The community identifier"),
      bedrooms: z.number().optional().describe("Number of bedrooms (optional filter)")
    }),
    func: async ({ community_id, bedrooms }) => {
      try {
        const results = await this.checkAvailability(community_id, bedrooms);
  ...
  /**
   * Tool for checking pet policy
   */
  checkPetPolicyTool = new DynamicStructuredTool({
    name: "check_pet_policy",
    description: "Check pet policy for a specific pet type in a community",
    schema: z.object({
      community_id: z.string().describe("The community identifier"),
      pet_type: z.string().describe("Type of pet (e.g., cat, dog, bird)")
    }),
    func: async ({ community_id, pet_type }) => {
      try {
        const result = await this.checkPetPolicy(community_id, pet_type);
        return JSON.stringify(result);
      } catch (error) {
        return `Error checking pet policy: ${error instanceof Error ? error.message : 'Unknown error'}`;
      }
    }
  });

  /**
   * Tool for getting pricing information
   */
  getPricingTool = new DynamicStructuredTool({
    name: "get_pricing",
    description: "Get pricing information for a specific unit",
    schema: z.object({
      community_id: z.string().describe("The community identifier"),
      unit_id: z.string().describe("The unit identifier"),
      move_in_date: z.string().optional().describe("Desired move-in date (optional)")
    }),
    func: async ({ community_id, unit_id, move_in_date }) => {
      try {
        const result = await this.getPricing(community_id, unit_id, move_in_date);
        return JSON.stringify(result);
      } catch (error) {
        return `Error getting pricing: ${error instanceof Error ? error.message : 'Unknown error'}`;
      }
    }
  });

  /**
   * Tool for getting available tour slots
   */
  getAvailableTourSlotsTool = new DynamicStructuredTool({
    name: "get_available_tour_slots",
    description: "Get available tour time slots for a community",
    schema: z.object({
      community_id: z.string().describe("The community identifier"),
      limit: z.number().default(5).describe("Maximum number of slots to return (default 5)")
    }),
    func: async ({ community_id, limit = 5 }) => {
      try {
        const slots = await this.getAvailableTourSlots(community_id, limit);
        return JSON.stringify(slots);
      } catch (error) {
        return `Error getting tour slots: ${error instanceof Error ? error.message : 'Unknown error'}`;
      }
    }
  });

  /**
   * Get all tools as an array for use with LangChain agents
   */
  getAllTools() {
    return [
      this.checkAvailabilityTool,
      this.checkPetPolicyTool,
      this.getPricingTool,
      this.getAvailableTourSlotsTool
    ];
  }

  // Legacy methods for backward compatibility - these will be used by the tools
  private async checkAvailability(community_id: string, bedrooms?: number): Promise<AvailabilityResult[]> {
    let query = `
  ...
  private async checkPetPolicy(community_id: string, pet_type: string): Promise<PetPolicyResult | null> {
    const query = `
  ...
  private async getPricing(community_id: string, unit_id: string, move_in_date?: string): Promise<PricingResult | null> {
    const query = `
  ...
  private async getAvailableTourSlots(community_id: string, limit: number = 5): Promise<string[]> {
    const query = `
      SELECT slot_time
      FROM tour_slots 
